...
void WrappedCandy::clearWithoutAnimation()
{
    ...
    containerCell->setContent(std::make_shared<WrappedCandySecPhase>(grid, containerCell, getCenter(), grid.getCellContentSide(), color));
...
WrappedCandySecPhase::WrappedCandySecPhase(
        Grid &grid,
        Cell *cell,
        Point center,
        int side,
...
void WrappedCandySecPhase::clearWithoutAnimation()
{
    StandardCandy::clearWithoutAnimation();
